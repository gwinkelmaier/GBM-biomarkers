...
def main():
    ...
    parser.add_argument(
        "--debug", dest="DEBUG", action="store_true", help="Sets log level to DEBUG"
    )
    parser.add_argument(
        "--threshold",
        dest="THRESH",
    ...
    image_names = [
        str(i)
        for i in (DATA_DIR / flags["WSI"] / "2021" / flags["PROB_DIR"]).glob("*.png")
    ]
    ...
    for image in image_names:
        ...
        df["output_name"] = (
            "patch_"
            + df["col_idx"].astype(str)
            + "_"
            + df["row_idx"].astype(str)
            + ".png"
        )
